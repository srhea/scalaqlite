    ...
    extends Iterator[IndexedSeq[SqlValue]]
{
    ...
    private def advance() {
        cachedRow = Sqlite3C.Instance.sqlite3_step(stmt) match {
            case Sqlite3C.ROW =>
                (0 until Sqlite3C.Instance.sqlite3_column_count(stmt)).map { i =>
                    Sqlite3C.Instance.sqlite3_column_type(stmt, i) match {
                        case Sqlite3C.INTEGER => SqlInt(Sqlite3C.Instance.sqlite3_column_int(stmt, i))
                        case Sqlite3C.FLOAT => SqlDouble(Sqlite3C.Instance.sqlite3_column_double(stmt, i))
                        case Sqlite3C.TEXT => SqlText(Sqlite3C.Instance.sqlite3_column_text(stmt, i))
                        case Sqlite3C.NULL => SqlNull()
                        case _ => sys.error("unsupported type")
                    }
                }
            case Sqlite3C.DONE =>
                Sqlite3C.Instance.sqlite3_finalize(stmt)
                null
            case Sqlite3C.ERROR =>
...
class SqliteDb(path: String) {
    ...
    Sqlite3C.Instance.sqlite3_open(path, db) ensuring (_ == Sqlite3C.OK, errmsg)
    def close() {
        assert(db.getValue != null, "already closed")
        Sqlite3C.Instance.sqlite3_close(db.getValue) ensuring (_ == Sqlite3C.OK, errmsg)
        db.setValue(null)
    }
    override def finalize() { if (db.getValue != null) Sqlite3C.Instance.sqlite3_close(db.getValue) }
    def query(sql: String): Iterator[IndexedSeq[SqlValue]] = {
        ...
        Sqlite3C.Instance.sqlite3_prepare_v2(db.getValue, sql, sql.length, stmt, tail) ensuring (_ == Sqlite3C.OK, errmsg)
        new SqliteResultIterator(this, stmt.getValue)
    }
    def execute(sql: String) { for (row <- query(sql)) () }
    def enableLoadExtension(on: Boolean) {
        Sqlite3C.Instance.sqlite3_enable_load_extension(db.getValue, if (on) 1 else 0)
    }
    def errmsg: String = if (db.getValue == null) "db not open" else Sqlite3C.Instance.sqlite3_errmsg(db.getValue)
}
